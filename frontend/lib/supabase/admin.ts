...
export function hasSupabaseAdminCredentials(): boolean {
  ...
  return Boolean(
    ...
    (serviceRoleKey.startsWith("sb_secret_") || isLegacyServiceRoleKey(serviceRoleKey))
  );
}
...
export function createSupabaseAdminClient() {
  ...
  if (!url || !serviceRoleKey || !hasSupabaseAdminCredentials()) {
    throw new Error("Supabase admin access requires a server-side secret or service-role key.");
  }
