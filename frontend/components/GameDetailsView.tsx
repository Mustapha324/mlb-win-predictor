    ...
    <main>
      ...
      <section className="relative mt-5 overflow-hidden rounded-[30px] border border-white/[0.08] bg-[#080808] p-6 sm:p-10">
        ...
        <div className="relative mt-10 grid gap-5 lg:grid-cols-[.8fr_1.2fr]">
          <div className="relative overflow-hidden rounded-[22px] border border-white/[0.09] bg-white/[0.03] p-6"><div ><p className="eyebrow">Pregame call · locked</p><p className="mt-2 text-2xl font-black text-white">{game.pregame_predicted_winner}</p><p className={`mt-1 font-mono text-4xl font-black ${accent}`}>{percent(pickProbability)}</p><p className="mt-2 text-xs font-bold uppercase tracking-[0.12em] text-neutral-500">{game.confidence} confidence{game.prediction_tier ? ` · Tier ${game.prediction_tier}` : ""}</p>{game.market_home_win_probability !== null && game.market_home_win_probability !== undefined ? <p className="mt-3 text-xs leading-5 text-neutral-500">Sportsbook line: {game.pregame_predicted_winner} {percent(homeIsPick ? game.market_home_win_probability : 1 - game.market_home_win_probability)} · model alone {percent(homeIsPick ? game.model_home_win_probability ?? pickProbability : 1 - (game.model_home_win_probability ?? 1 - pickProbability))}</p> : null}</div></div>
          <div className="relative overflow-hidden rounded-[22px] border border-white/[0.07] bg-black/20 p-6"><div ><p className="eyebrow">Why the model leans this way</p><ul className="mt-4 space-y-3">{(game.factors.length ? game.factors : ["Detailed factors are not available for this game yet."]).map((factor, index) => <li key={`${factor}-${index}`} className="flex gap-3 text-sm text-neutral-300"><span className={`font-mono ${accent}`}>0{index + 1}</span><span>{factor}</span></li>)}</ul></div></div>
        </div>

        {game.is_final && game.actual_winner ? <div className="mt-5 rounded-[22px] border border-white/[0.09] bg-white/[0.035] p-6"><p className="eyebrow">Verified final winner</p><div className="mt-2 flex flex-wrap items-center justify-between gap-3"><div><p className="text-2xl font-black text-white">{game.actual_winner}</p>{game.home_score !== null && game.away_score !== null ? <p className="mt-1 font-mono text-sm text-neutral-400">{game.awayTeam.abbreviation} {game.away_score} · {game.homeTeam.abbreviation} {game.home_score}</p> : null}</div>{<span className={game.actual_winner === game.pregame_predicted_winner ? "text-emerald-300" : "text-rose-300"}>{game.actual_winner === game.pregame_predicted_winner ? "Pregame pick hit" : "Pregame pick missed"}</span>}</div></div> : null}
        {!game.is_final && game.pick_leading !== null && game.pick_leading !== undefined ? <div className={`mt-5 rounded-[22px] border p-6 ${game.pick_leading ? "border-emerald-300/20 bg-emerald-300/[0.05]" : "border-amber-300/20 bg-amber-300/[0.05]"}`}><p className="eyebrow">Live check</p><p className={`mt-2 text-xl font-black ${game.pick_leading ? "text-emerald-200" : "text-amber-200"}`}>{game.pregame_predicted_winner} is {game.pick_leading ? "leading" : "trailing"}{game.home_score !== null && game.away_score !== null ? ` · ${game.awayTeam.abbreviation} ${game.away_score}, ${game.homeTeam.abbreviation} ${game.home_score}` : ""}</p></div> : null}
